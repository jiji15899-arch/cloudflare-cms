...
${urls}
  ...
  async function route(request, env, ctx) {
    ...
    const path = url.pathname.replace(/\/+$/, "") || "/";
    const method = request.method.toUpperCase();
    if (path.startsWith("/cp-includes/") || path.startsWith("/cp-config") || path.startsWith("/cp-settings") || path.startsWith("/cp-load") || path.startsWith("/node_modules/")) {
      return forbidden();
    }
    if (path === "/cp-admin/setup-config" || path === "/cp-admin/install") {
      return handleInstaller(request, env, ctx);
...
var __handler = __CP_WORKER__["default"];

addEventListener('fetch', function(event) {
  event.respondWith(
    __handler.fetch(event.request, globalThis, event)
  );
});

addEventListener('scheduled', function(event) {
  event.waitUntil(
    __handler.scheduled(event, globalThis, event)
  );
});
